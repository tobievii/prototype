...
export function webapi_v4() {
    ...
    describe("PROTOTYPE V4 API MQTT", () => {
        ...
        it("HTTP -> MQTT (APIKEY WIDE SUB)", function (done) {
            ...
            client.on('connect', () => {
                client.subscribe(account.apikey, { qos: 1 }, (err: Error) => {
                    ...
                    request.post(uri + "/api/v4/data/post", { headers, json: packet })
        ...
        it("HTTP -> MQTT (PER DEVICE SUB)", function (done) {
            ...
            client.on('connect', () => {
                client.subscribe(account.apikey + "|" + packet.id, { qos: 1 }, (err: Error) => {
                    ...
                    request.post(uri + "/api/v4/data/post", { headers, json: dummypacket }, () => {
        ...
        it("MQTT -> HTTP", done => {
            ...
            client.on('connect', function () {
                client.publish(account.apikey, JSON.stringify(packet), { qos: 1 }, () => {
                    request.post(uri + "/api/v4/view", { headers, json: { id: packet.id } }, (err, res, response) => {
                        if (err) done(err);
                        if (response) {
