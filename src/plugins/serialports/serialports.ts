  ...
  connectDevices() {
    ...
    for (var device in this.listOfPluggedInDevices) {
      ...
      for (var port in this.listofConnectedPorts) {
        if (this.listOfPluggedInDevices[device].comName == this.listofConnectedPorts[port].device.comName) {
          alreadyPlugged = true;
        }
      }
      ...
      if (alreadyPlugged === false) {
        ...
        var dev = new SerialJSONDevice(this.listOfPluggedInDevices[device], { baudRate: 115200 })
        ...
        dev.on("close", (err) => {
          ...
          this.listofConnectedPorts = this.listofConnectedPorts.filter((device: any) => { device.comName == dev.device.comName })
...
export function init(app: any, db: any, eventHub: events.EventEmitter) {
      ...
      if (config.apikey) {
        ...
        try {
          if (data.datajson.id) {
            ...
            eventHub.emit("device", { apikey: config.apikey, packet: { id : data.datajson.id, level:1, data: data.datajson.data, serial: data , meta: { method: "serialport" } } } )

            try {
                var rx = JSON.parse(data.datajson.rx.buf)

                if (rx.id) {
                  eventHub.emit("device", { apikey: config.apikey, packet: { id : rx.id, level: 2, data: rx.data, serial: data, meta: { method: "serialportRxTx" } } } )
                }
  ...
  sp.on("close", (data) => {
    ...
    if (error == "Error: bad file descriptor") { error = "Unplugged" }
...
function setserialportaccount(db:any, device:any, user:any, cb:any) {
  db.plugins_serialports.update({pnpId:device.pnpId}, 
    {"$set":{ "apikey" : user.apikey}}, (err:Error, resultUpd:any)=>{
...
function updateserialportdevice(db:any, device:any, cb:any) {
  db.plugins_serialports.findOne({pnpId:device.pnpId}, (err:any, devicedb:any)=>{
    var newdevice:any = {}
    
    if (devicedb == null) {      newdevice = _.clone(device);
    } else {      newdevice = _.merge(devicedb, device)
    }

    db.plugins_serialports.update({pnpId:newdevice.pnpId}, newdevice, {upsert:true}, (err:any, res:any)=>{
      //console.log(res);
    })
